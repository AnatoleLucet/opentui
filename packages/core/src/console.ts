...
export class TerminalConsole extends EventEmitter {
  ...
  private _isDraggingHeader: boolean = false;
  ...
  constructor(renderer: CliRenderer, options: ConsoleOptions = {}) {
    ...
    this._updateConsoleDimensions(undefined, {
      width: Math.floor((this.renderer.width * this.options.sizePercent) / 100),
      height: Math.floor((this.renderer.height * this.options.sizePercent) / 100),
    });
  ...
  private _updateConsoleDimensions(
    ...
    const containerWidth = container?.width ?? this.renderer.width;
    const containerHeight = container?.height ?? this.renderer.height;
    const consoleWidth = content?.width ?? this.consoleWidth;
    const consoleHeight = content?.height ?? this.consoleHeight;

    switch (this.options.position) {
      case ConsolePosition.TOP:
        this.consoleX = 0;
        this.consoleY = 0;
        this.consoleWidth = containerWidth;
        this.consoleHeight = Math.max(1, Math.min(containerHeight, consoleHeight));
        break;
      case ConsolePosition.BOTTOM:
        this.consoleHeight = Math.max(1, Math.min(containerHeight, consoleHeight));
        this.consoleWidth = containerWidth;
        this.consoleX = 0;
        this.consoleY = containerHeight - this.consoleHeight;
        console.log("new height", this.consoleHeight, "new y", this.consoleY);
        break;
      case ConsolePosition.LEFT:
        this.consoleWidth = Math.max(1, Math.min(containerWidth, consoleWidth));
        this.consoleHeight = containerHeight;
        this.consoleX = 0;
        this.consoleY = 0;
        break;
      case ConsolePosition.RIGHT:
        ...
        this.consoleHeight = containerHeight;
        this.consoleY = 0;
        this.consoleX = containerWidth - this.consoleWidth;
        break;
    }
  ...
  public handleMouse(event: MouseEvent): boolean {
    ...
    // console.log("localY:", localY, this._isDraggingHeader);

    // handle header
    if (localY === 0 || this._isDraggingHeader) {
      if (event.type === "down" && event.button === 0) {
        this._isDraggingHeader = true;

        // handle copy button click
        if (localX >= this._copyButtonBounds.x && localX < this._copyButtonBounds.x + this._copyButtonBounds.width) {
          this.triggerCopy();
          return true;
        }
      }
      ...
      if (this._isDraggingHeader && event.type === "drag") {
        if (this.options.position == ConsolePosition.BOTTOM) {
          this._updateConsoleDimensions(undefined, { height: this.consoleHeight + localY * -1 });
          this.markNeedsRerender();
        }
  ...
  public get isDragging(): boolean {
    return this._isDragging || this._isDraggingHeader;
  }
